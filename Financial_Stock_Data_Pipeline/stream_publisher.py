...
with open('./source_data/aapl_streaming_apr.csv', 'r') as csvfile:
    ...
    for row in spamreader:
        try:
            json_dict['date'] = row[0]
            json_dict['open'] = float(row[1])
            json_dict['high'] = float(row[2])
            json_dict['low'] = float(row[3])
            json_dict['close'] = float(row[4])
            json_dict['adj_close'] = float(row[5])
            json_dict['volume'] = int(row[6])
